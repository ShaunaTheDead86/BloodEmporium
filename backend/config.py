...
class Config:
    def __init__(self, validate=True):
        ...
        for num, profile in enumerate(self.config["profiles"], 1):
            if "id" not in profile.keys():
                raise ConfigError(
                    f"Missing profile id for profile {num} in config.json"
                )
            if profile["id"] in ids:
                raise ConfigError(
                    f"Multiple profiles with same id (profile {num}) in config.json"
                )
            ...
            for unique_id, v in profile.items():
                ...
                if unique_id not in ["id", "notes"]:
                    ...
                    if "subtier" not in v:
                        ...
                        continue
            [
                profile.pop(invalid_unlockable)
                for invalid_unlockable in invalid_unlockables
            ]

    def top_left(self):
        return self.config["capture"]["top_left_x"], self.config["capture"][
            "top_left_y"
        ]
    ...
    def get_profile_by_id(self, profile_id):
        ...
        return (
            [p for p in profiles if p["id"] == profile_id].pop(0)
            if len(profiles) > 0
            else {"id": None, "notes": ""}
        )
    ...
    def preference_by_id(self, unlockable_id, profile_id):
        if profile_id is None:
            return 0, 0
        return self.preference_by_profile(
            unlockable_id, self.get_profile_by_id(profile_id)
        )
                ...
                json.dump(
                    {
                        ...
                        "interaction": self.config.get(
                            "interaction", default_config["interaction"]
                        ),
                        "primary_mouse": self.config.get(
                            "primary_mouse", default_config["primary_mouse"]
                        ),
                        "profiles": self.config.get(
    ...
    def set_profile(self, updated_profile):
        ...
        self.config["profiles"][
            self.config["profiles"].index(self.get_profile_by_id(updated_profile["id"]))
        ] = updated_profile
        self.commit_changes()
    ...
    def delete_profile(self, profile_id):
        to_be_removed = [
            profile for profile in self.__profiles() if profile["id"] == profile_id
        ].pop(0)
