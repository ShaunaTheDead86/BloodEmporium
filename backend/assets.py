...
if __name__ == "__main__":
    all_files = [
        (subdir, file)
        for subdir, dirs, files in os.walk(Path.vanilla_image)
        for file in files
    ]
        ...
        for subdir, file in all_files:
            if unlockable.id == file[:-4]:
                ...
                shutil.copyfile(
                    ...
                    Path.assets_file(unlockable.category, unlockable.id),
                )
                found = True
                break
        if not found:
            print(
                f"no source found for desired unlockable: {unlockable.id} under category: {unlockable.category}"
            )
