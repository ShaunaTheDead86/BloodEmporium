...
from yolov5_obb.utils.general import (
    check_img_size,
    scale_polys,
...
class EdgeDetection:
    def __init__(self):
        # load model on init
        self.model = DetectMultiBackend("models/edges v3.pt")
        self.model.float()
    ...
    def predict(self, img_original):
        ...
        results = non_max_suppression_obb(
            results, 0.6, 0.45, None, False, multi_label=True, max_det=100
        )
        ...
        if len(results) > 0:
            ...
            pred_poly = scale_polys(
                img_rescaled.shape[2:], pred_poly, img_original.shape
            )
            processed_results = torch.cat(
                (pred_poly, results[:, -2:]), dim=1
            )  # (n, [poly conf cls])
    ...
    def link_edges(
        self, results, matched_nodes: List[MatchedNode], avg_diameter
    ) -> List[LinkedEdge]:
        ...
        for (
            ...
            confidence,
            cls,
        ) in results:  # cls not useful
            closests = []
            for x, y in [(x1, y1), (x2, y2), (x3, y3), (x4, y4)]:
                closest = min(
                    matched_nodes, key=lambda node: node.box.centre().distance_xy(x, y)
                )
                if (
                    closest not in closests
                    and closest.box.centre().distance_xy(x, y) < avg_diameter
                ):
