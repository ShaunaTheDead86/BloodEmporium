...
from backend.shapes import MatchedNode
from backend.util.node_util import ColorUtil
from backend.util.network_util import NetworkUtil
...
class Debugger:
    ...
    def set_image(self, bloodweb_iteration, cv_image: CVImage):
        ...
        if self.write_to_output:
            ...
            cv2.imwrite(
                f"output/{self.time}/{bloodweb_iteration}/initial.png",
                cv_image.get_bgr(),
            )
    ...
    def set_base_bloodweb(self, bloodweb_iteration, base_bloodweb):
        if self.write_to_output:
            NetworkUtil.write_to_html(
                base_bloodweb, f"output/{self.time}/{bloodweb_iteration}/base_bloodweb"
            )
        return self
    ...
    def add_updated_image(self, bloodweb_iteration, update_iteration, updated_image):
        if self.write_to_output:
            cv2.imwrite(
                f"output/{self.time}/{bloodweb_iteration}/updated_{update_iteration}.png",
    ...
    def construct_and_show_images(self, bloodweb_iteration):
        ...
        for node in self.nodes[bloodweb_iteration]:
            cv2.rectangle(
                ...
                node.box.nw.xy(),
                node.box.se.xy(),
                ColorUtil.bgr_from_cls_name(node.cls_name),
                4,
            )

            if node.unique_id != "":
                matched_icon = self.merged_base.valid_images[
                    self.merged_base.names.index(node.unique_id)
                ]
                resized_match = cv2.resize(
                    matched_icon,
                    tuple(reversed([c * 2 // 3 for c in matched_icon.shape])),
                )
                centre_x, centre_y = node.box.centre().xy()
                node_width, node_height = node.box.dimensions()
                offset_x, offset_y = (
                    centre_x + node_width // 2,
                    centre_y - node_height // 2,
                )
                img[
                    offset_y : offset_y + resized_match.shape[0],
                    offset_x : offset_x + resized_match.shape[1],
